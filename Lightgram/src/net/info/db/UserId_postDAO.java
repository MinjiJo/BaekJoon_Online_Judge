...
public class UserId_postDAO {
	...
	public List<UserId_post> getList(String id, String search, String search_option){
		...
		try {
			...
			String sql = "select id from All_users where " + search_option + " like ?";
			pstmt = conn.prepareStatement(sql);
		...
		try {
			...
			sql = "select b.* "
				...
				while(!target.isEmpty()){
					String targetId = target.peek().split("_")[0];
					sql += "select * from " + targetId + "_post where itemNum = " + Integer.parseInt(target.peek().split("_")[1]) + " and openlevel <";
					if(targetId.equals(id)) {
						sql += "4 union ";
						target.poll();
					}else if(id_follow_list.contains(target.poll().split("_")[0]))
						sql += "3 union ";
					else
						sql += "2 union ";
				}
