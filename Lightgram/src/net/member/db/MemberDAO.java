...
public class MemberDAO {
	...
	public int isId(String checkId) {	// id泥댄겕�떆 �뾾�쑝硫� 0, �엳�쑝硫� -1
		try {
			conn = ds.getConnection();
	...
	public int getListCount() { //admin�슜
		try {
			conn = ds.getConnection();
	...
	public int getListCount(String field, String value) { //admin�슜
		int x = 0;
		try {
			conn = ds.getConnection();
	...
	public List<Member> searchAll(String field, String value, int page, int limit) { //admin�슜
		String sql = "select * "
		...
		try {
			...
			rs = pstmt.executeQuery();
			// DB�뿉�꽌 媛��졇�삩 �뜲�씠�꽣瑜� VO媛앹껜�뿉 �떞�뒿�땲�떎.
			while (rs.next()) {
				Member m = new Member();
			...
			return list; // 媛믪쓣 �떞�쓣 媛앹껜瑜� ���옣�븳 由ъ뒪�듃瑜� �샇異쒗븳 怨녹쑝濡� 媛��졇媛묐땲�떎.
		} catch (SQLException ex) {
			System.out.println("searchAll(4) �뿉�윭 : " + ex);
			ex.printStackTrace();
		} finally {
			close();
		}
	...
	public List<Member> searchAll(int page, int limit) { //admin�슜
		String sql = "select * "
					+ "from (select rownum rnum, b.* "
					+ "from (select * from All_users "
					+ "where id != 'admin' "
	...
	public int isEmail(String checkemail) {	
		try {
			conn = ds.getConnection();
			String sql = "select id from All_users where email = ?";
			pstmt = conn.prepareStatement(sql);
