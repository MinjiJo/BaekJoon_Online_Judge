...
public class MemberDAO {
	...
	public int join(Member member) {
		...
		try {
			...
			String sql = "insert into All_users (id, password, name, email) values(?, ?, ?, ?)";
	...
	public int delete(String id) {
		...
		try {
			...
			String sql = "drop table "+id+"_post_like";
			pstmt = conn.prepareStatement(sql);
			pstmt.execute();
	...
	public int followingcount(String id) {
		int x = 0;
		try {
			String sql = "select count(id) from " + id + "_follow";
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if(rs.next()) {
	...
	public List<Member> searchAll(String field, String value, int page, int limit) {
		String sql = "select * "
	...
	public List<Member> searchAll(int page, int limit) {
		String sql = "select * "
					+ "from (select rownum rnum, b.* "
					+ "from (select * from All_users "
					+ "where id != 'admin' "
