...
//					clicked.parent().empty();
	...
	function getList(){
						...
						output += '<div class="more">'
									...
									+ '<table class="table table-borderless" style="width:760px">'
										...
										+ 	'<th width="85%">'
											...
											+ '<img src="id/' + this.id + '/' + this.profileImg + '" style="border-radius: 50px; width:50px;"></a><div class="header_id">'
		...
		output								+= '</div></th>'
						...
						if(this.id != rdata[2] && this.id != 'admin'){
							...
							output += "'postReport.do?id=" + this.id + "&&itemNum=" + this.itemNum + "'";
							output += '">신고';
						} else {
							output += '<div onClick="location.href=';
						...
						output		+='<tbody>'
										+'<tr>'
											+'<td colspan="2" style="padding:0" ><img src="id/' + this.id + '/' + this.itemNum + '/' + this.pic_url + '" style="width:760px;"></td>'
										+'</tr>'
										+'<tr>'		
					...
					output				+='</tr>';
					count = 0;
					$.each(this.replied, function(indexes, items){
						count = count + 1;
						if(indexes < 3){
							output += '<tr class="comment_row">'
										+'<td><a href="Search.do?search_word=' + this.reply_id + '&&search_option=id_sel" style="color: black; text-decoration: none;">'
											+'<img src="id/' + this.reply_id + '/' +this.profileImg + '" style="width:15px;margin-right:5px;">' + this.reply_id + '</a>' 
											+ '&nbsp;&nbsp;&nbsp;&nbsp;' + this.comments  + '</td>'
	...
	$('.moremore').on('keydown', '.input_reply', function(){
		var keys = $(this).val();
		if(keys != null){
			...
			$(this).parent().next().children().css('cursor', 'none');
		}
	});
